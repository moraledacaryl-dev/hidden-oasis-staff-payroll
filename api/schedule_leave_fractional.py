...
def save_day_leave(payload: DayLeavePayload, authorization: str | None = Header(default=None, alias="Authorization"), x_api_key: str | None = Header(default=None, alias="X-API-Key")) -> dict[str, Any]:
    ...
    try:
        ...
        leave_days = validate_day_editor_leave_fraction(payload.leave_days, payload.leave_hours, scheduled_hours) if leave_kind in DAY_EDITOR_LEAVE_TYPES else 1.0
        timestamp = now_iso()
        ...
        if leave_kind == "None":
            ...
            if existing_log:
                conn.execute("UPDATE time_logs SET is_absent=1, absence_type=?, attendance_status=?, reviewed_by=?, reviewed_at=?, notes=?, notice_given_at=?, notice_timing=?, evidence_ref=?, updated_at=? WHERE id=?", (leave_kind, attendance_status, user.get("display_name"), timestamp, payload.reason, notice_given_at, notice_timing, payload.evidence_ref, timestamp, existing_log["id"]))
                log_id = int(existing_log["id"])
            else:
                cur = conn.execute("INSERT INTO time_logs(employee_id, work_date, source, verification_type, is_absent, absence_type, detected_ot_hours, approved_ot_hours, ot_status, attendance_status, reviewed_by, reviewed_at, notes, notice_given_at, notice_timing, evidence_ref, created_at, updated_at) VALUES (?, ?, 'manual', 'Manual', 1, ?, 0, 0, 'None', ?, ?, ?, ?, ?, ?, ?, ?, ?)", (employee_id, shift_date, leave_kind, attendance_status, user.get("display_name"), timestamp, payload.reason, notice_given_at, notice_timing, payload.evidence_ref, timestamp, timestamp))
            ...
            if leave_kind in DAY_EDITOR_LEAVE_TYPES:
                ...
                existing_leave = fetch_leave(conn, employee_id, shift_date)
                if existing_leave:
                    conn.execute("UPDATE leave_requests SET leave_type_id=?, start_date=?, end_date=?, days=?, paid=?, status='Approved', reason=?, reviewed_by=?, reviewed_at=? WHERE id=?", (leave_type_id, shift_date, shift_date, leave_days, paid, payload.reason, user.get("display_name"), timestamp, existing_leave["id"]))
                else:
                    conn.execute("INSERT INTO leave_requests(employee_id, leave_type_id, start_date, end_date, days, paid, status, reason, reviewed_by, reviewed_at, created_at) VALUES (?, ?, ?, ?, ?, ?, 'Approved', ?, ?, ?, ?)", (employee_id, leave_type_id, shift_date, shift_date, leave_days, paid, payload.reason, user.get("display_name"), timestamp, timestamp))
        if log_id:
            after = dict(fetchone(conn, "SELECT * FROM time_logs WHERE id=?", (log_id,)) or {})
            log_schedule_change(conn, change_type="update_absence" if before else "create_absence", entity_type="time_log", entity_id=log_id, employee_id=employee_id, work_date=shift_date, before=before, after=after, changed_by=user.get("display_name"))
        conn.commit()
