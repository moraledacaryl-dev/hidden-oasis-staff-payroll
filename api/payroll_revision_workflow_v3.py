from api.payroll_revision_workflow_v2 import *
from api.payroll_revision_workflow_v2 import router as _old_router
...
def carry_forward_manual_values_v3(conn, source_run_id: int, new_run_id: int, treatment: str, stamp: str, actor: str | None) -> int:
    ...
    for adjustment in source_adjustments:
        ...
        if treatment == "replace_unpaid" and adjustment.get("cash_advance_id") and cash > 0:
            payout = fetchone(conn, "SELECT payout_date FROM payroll_runs WHERE id=?", (new_run_id,)) or {}
            conn.execute("""
            ...
            """, (
                ...
                payout.get("payout_date"), cash, "Payroll", "Payroll deduction",
                new_run_id, item["id"], actor, stamp, actor, stamp,
            ))
        copied += 1
...
def save_controlled_revision_v3(
    run_id: int,
    ...
    try:
        ...
        copied_adjustments = carry_forward_manual_values_v3(conn, run_id, new_run_id, payload.treatment, stamp, user.get("display_name"))
        ...
        if payload.treatment == "adjust_paid":
            ...
            for employee_id in sorted(set(original_items) | set(revised_items)):
                ...
                conn.execute("INSERT INTO payroll_revision_adjustments(revision_run_id,original_run_id,employee_id,original_net_pay,revised_net_pay,adjustment_amount,adjustment_direction,created_at) VALUES(?,?,?,?,?,?,?,?)", (new_run_id, run_id, employee_id, original_net, revised_net, difference, direction, stamp))
...
def get_revision_adjustments_v3(
    run_id: int,
    authorization: str | None = Header(default=None, alias="Authorization"),
    x_api_key: str | None = Header(default=None, alias="X-API-Key"),
) -> dict[str, Any]:
    ...
    try:
        ...
        if not run:
            raise HTTPException(status_code=404, detail="Payroll run not found.")
        items = fetchall(conn, "SELECT pra.*,e.full_name,e.employee_code FROM payroll_revision_adjustments pra LEFT JOIN employees e ON e.id=pra.employee_id WHERE pra.revision_run_id=? ORDER BY e.full_name,pra.employee_id", (run_id,))
