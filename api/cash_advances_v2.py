...
def ensure_schema(conn) -> None:
    ...
    """)
    cols = _columns(conn, "cash_advances")
    additions = {
        "advance_date": "TEXT",
        "reason": "TEXT",
        "approved_by": "TEXT",
    ...
    if "repayment_per_cutoff" in cols:
    ...
    conn.execute("UPDATE cash_advances SET remaining_balance=ledger_opening_balance WHERE remaining_balance IS NULL")
...
def recalculate_balance(conn, cash_advance_id: int) -> dict[str, Any]:
    ...
    status = "Cancelled" if row.get("status") == "Cancelled" else ("Fully Paid" if balance <= 0 else "Active")
    ...
    return {"amount": round(float(row.get("amount") or 0),2), "historical_paid": historical_paid, "new_paid": new_paid, "paid": total_paid, "balance": balance, "status": status}
...
def save_cash_advance(payload: CashAdvancePayload, authorization: str | None = Header(default=None, alias="Authorization"), x_api_key: str | None = Header(default=None, alias="X-API-Key")) -> dict[str, Any]:
    ...
    try:
        ...
        if payload.id:
            ...
            if not old:
                raise HTTPException(status_code=404, detail="Cash advance not found.")
            employee_id = payload.employee_id or int(old["employee_id"])
            ...
            conn.execute("""UPDATE cash_advances SET employee_id=?,advance_date=?,amount=?,reason=?,approved_by=?,repayment_method=?,deduction_per_payroll=?,ledger_opening_balance=?,status=?,notes=?,updated_by=?,updated_at=? WHERE id=?""", (employee_id,payload.advance_date,amount,payload.reason,payload.approved_by,method,round(float(payload.deduction_per_payroll or 0),2),adjusted_opening,payload.status,payload.notes,user.get("display_name"),stamp,payload.id))
