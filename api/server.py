...
from api.main import API_PREFIX, PayrollPreviewRequest, build_app as build_core_app, configured_db_path, db_conn, parse_date_order, payroll_result_to_api
...
def initialize_runtime() -> None:
    validate_runtime_environment(); conn = get_conn(configured_db_path())
    try:
        init_db(conn); ensure_schedule_schema(conn); ensure_schedule_change_log_schema(conn); ensure_workflow_schema(conn); ensure_integration_schema(conn); ensure_legacy_integration_writer_compatibility(conn); conn.commit()
    finally: conn.close()


@asynccontextmanager
async def application_lifespan(_: FastAPI) -> AsyncIterator[None]:
    initialize_runtime(); yield
...
def _route_key(route: Any) -> tuple[str, str] | None:
    methods, path = getattr(route, "methods", None), getattr(route, "path", None)
    return (str(path), ",".join(sorted(str(method).upper() for method in methods))) if path and methods else None
...
def _walk_routes(routes: list[Any]) -> list[Any]:
    ...
    for route in routes:
        ...
        if nested: result.extend(_walk_routes(list(nested)))
    return result


def _route_method_keys(route: Any) -> list[tuple[str, str]]:
    path, methods = getattr(route, "path", None), getattr(route, "methods", None)
    return [(str(path), str(method).upper()) for method in methods] if path and methods else []


def _route_endpoint_name(route: Any) -> str:
    endpoint = getattr(route, "endpoint", None); return getattr(endpoint, "__name__", repr(endpoint))
...
def assert_unique_route_registry(application: FastAPI) -> None:
    ...
    for route in _walk_routes(list(application.router.routes)):
        for key in _route_method_keys(route): by_method[key].append(_route_endpoint_name(route))
    duplicates = {key: endpoints for key, endpoints in by_method.items() if len(endpoints) > 1}
    if duplicates:
        details = "; ".join(f"{method} {path} -> {', '.join(endpoints)}" for (path, method), endpoints in sorted(duplicates.items()))
...
def _include_router_filtered(application: FastAPI, source: APIRouter, excluded: set[tuple[str, str]]) -> None:
    filtered = APIRouter(); filtered.routes.extend(route for route in source.routes if _route_key(route) not in excluded); application.include_router(filtered)


SCHEDULES_EXCLUDED_ROUTES = {(f"{API_PREFIX}/schedules/day/leave", "POST"), (f"{API_PREFIX}/schedules/shifts/{{shift_id}}/delete", "POST"), (f"{API_PREFIX}/schedules/shifts/{{shift_id}}/move", "POST"), (f"{API_PREFIX}/schedules/week", "GET")}
ROUTERS = (impersonation_router, payroll_drafts_router, payroll_return_router, payroll_review_router, my_payroll_router, payroll_mark_paid_router, payroll_corrections_router, payroll_audit_events_router, schedule_migration_router, revision_controls_router, revision_workflow_router, production_health_router, hr_records_router, payslip_distribution_router, schedule_leave_fractional_router, schedule_actuals_router, schedule_rest_days_router, schedule_leave_statuses_router, sil_leave_router, users_router, employees_router, holidays_router, integrations_router, schedule_publication_router, schedule_canonical_runtime_router, staff_published_portal_router, attendance_compliance_router, attendance_template_import_router, cash_advances_router, cash_repayments_router, cash_advance_corrections_router, performance_reviews_router, payroll_adjustments_router, payroll_recalculate_router)
LEGACY_MAIN_OVERRIDES = {(f"{API_PREFIX}/schedules/day/scheduled", "POST"), (f"{API_PREFIX}/schedules/day/actual", "POST"), (f"{API_PREFIX}/schedules/day/leave", "POST")}


def _remove_superseded_routes(application: FastAPI) -> None:
    application.router.routes = [route for route in application.router.routes if not (getattr(route, "path", "") == f"{API_PREFIX}/payroll/preview" and "POST" in getattr(route, "methods", set())) and not any(getattr(route, "path", "") == path and method in getattr(route, "methods", set()) for path, method in LEGACY_MAIN_OVERRIDES)]


def create_app() -> FastAPI:
    application = build_core_app(); application.router.lifespan_context = application_lifespan; _remove_superseded_routes(application)
    application.add_api_route(f"{API_PREFIX}/payroll/preview", payroll_preview_fractional, methods=["POST"], dependencies=[Depends(require_api_key)])
    for router in ROUTERS: application.include_router(router)
    _include_router_filtered(application, schedules_router, SCHEDULES_EXCLUDED_ROUTES); application.include_router(staff_self_service_router); assert_unique_route_registry(application); return application
