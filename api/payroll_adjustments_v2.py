...
def save_adjustments(run_id: int, employee_id: int, payload: AdjustmentPayload, authorization: str | None = Header(default=None, alias="Authorization"), x_api_key: str | None = Header(default=None, alias="X-API-Key")) -> dict[str, Any]:
    ...
    earning, other, cash = [round(float(v or 0), 2) for v in (payload.additional_earning, payload.other_deduction, payload.cash_advance_amount)]
    ...
    try:
        ...
        if payload.cash_advance_id:
            ...
            available = recalculate_balance(conn, int(payload.cash_advance_id))["balance"] - reserved(conn, int(payload.cash_advance_id), run_id) + (oc if old_advance == payload.cash_advance_id else 0)
