...
PAID_LEAVE_TYPES = {"SIL", "Sick Leave", "Vacation Leave", "Bereavement Leave", "Official Business", "Other Approved Absence"}
    ...
    try:
        ...
        if existing_leave:
            conn.execute(
                ...
                WHERE id=?
                """,
                (leave_type_id, shift_date, shift_date, leave_days, paid, payload.reason, user.get("display_name"), timestamp, existing_leave["id"]),
            )
            ...
            cursor = conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, 'Approved', ?, ?, ?, ?)
                """,
                (payload.employee_id, leave_type_id, shift_date, shift_date, leave_days, paid, payload.reason, user.get("display_name"), timestamp, timestamp),
            )
        ...
        if existing_log:
            conn.execute(
                ...
                WHERE id=?
                """,
                (user.get("display_name"), timestamp, payload.reason, payload.notice_given_at, payload.notice_timing or None, payload.evidence_ref, timestamp, existing_log["id"]),
            )
            log_id = int(existing_log["id"])
        else:
            cursor = conn.execute(
                """
                INSERT INTO time_logs(employee_id, work_date, source, verification_type, is_absent, absence_type,
                    detected_ot_hours, approved_ot_hours, ot_status, attendance_status, reviewed_by, reviewed_at,
                    notes, notice_given_at, notice_timing, evidence_ref, created_at, updated_at)
                VALUES (?, ?, 'manual', 'Manual', 0, NULL, 0, 0, 'None', 'Approved', ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (payload.employee_id, shift_date, user.get("display_name"), timestamp, payload.reason, payload.notice_given_at, payload.notice_timing or None, payload.evidence_ref, timestamp, timestamp),
            )
