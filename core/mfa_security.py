...
def _fernet():
    ...
    if not secret:
        raise RuntimeError("STAFF_PAYROLL_MFA_KEY must be configured.")
...
def decrypt_mfa_secret(stored: str | None) -> str | None:
    ...
    if not stored.startswith(MFA_SECRET_PREFIX):
        if _production():
            raise RuntimeError(
                "Legacy plaintext MFA secret refused in production. "
                "Migrate it to encrypted storage before enabling the account."
            )
        return stored
