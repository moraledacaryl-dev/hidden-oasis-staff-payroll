...
def compute_employee_payroll(conn: sqlite3.Connection, emp: dict[str, Any], period_start: str, period_end: str) -> PayrollResult:
    ...
    if not is_freelance:
        ...
        for log in logs:
            ...
            nd = compute_nd_hours(log["work_date"], log.get("actual_in"), log.get("actual_out"), break_mins=0)
        ...
        for lr in leave_rows:
            ...
            d = overlap_start
            while d <= overlap_end:
                approved_leave_dates.add(d.isoformat())
                overlap_calendar_days += 1
                d += timedelta(days=1)
            requested_days = float(lr.get("days") or 0)
            total_calendar_days = max(1, (leave_end - leave_start).days + 1)
            paid_days_in_cutoff = min(requested_days, requested_days * (overlap_calendar_days / total_calendar_days)) if overlap_calendar_days else 0
