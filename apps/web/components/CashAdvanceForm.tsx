...
export function CashAdvanceForm({
  ...
  if (item && !canEditExisting) {
    return <span className="muted">Owner/payroll only</span>;
  }
  ...
  if (!open) {
    return <button className="button small" type="button" onClick={() => setOpen(true)}>Edit</button>;
  }
