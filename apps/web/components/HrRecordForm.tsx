"use client";
import { todayInManilaIso } from "@/lib/period";
...
export function HrRecordForm({ employees }: { employees: Employee[] }) {
  ...
  async function submit(formData: FormData) {
    ...
    const payload = {
      ...
      record_date: String(formData.get("record_date") || todayInManilaIso()),
    ...
    const res = await clientRequest("/api/hr/records", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; void submit(new FormData(form)).then((saved) => { if (saved) { form.reset(); setRecordType("Memo"); } }); }} className="form-grid">
      ...
      <label>Type<select name="record_type" value={recordType} onChange={(event) => setRecordType(event.target.value)}>{recordTypes.map((type) => <option key={type}>{type}</option>)}</select></label>
      <label>Date<input name="record_date" type="date" defaultValue={todayInManilaIso()} required /></label>
      ...
      {recordType === "Annual Review" ? <><label>Rating<input name="rating" type="number" min="0" max="5" step="0.1" placeholder="Annual review only" /></label>
      <label>Review period start<input name="review_period_start" type="date" /></label>
      <label>Review period end<input name="review_period_end" type="date" /></label></> : null}
      ...
      <div className="action-row form-span-full"><button className="primary-button" type="submit" disabled={busy}>{busy ? "Saving..." : "Save HR record"}</button></div>
