...
export function StaffSelfServicePanel(){
  ...
  useEffect(()=>{
    Promise.all([
      fetch("/api/schedule/day?self_service=1",{cache:"no-store"}),
  ...
  return <>
    <StaffShiftRequests employeeId={data.employee.id} schedule={data.schedule||[]} requests={data.requests||[]} coworkers={data.coworkers||[]}/>
