...
export function PayrollLifecycleButtons({ runId, status }: { runId: number; status: string }) {
  ...
  return (
    <div className="action-row">
      ...
      {status === "For Owner Review" ? <button className="button small" disabled={!!busy} onClick={() => submit("approve")}>{busy === "approve" ? "Approving..." : "Approve"}</button> : null}
      {(status === "For Owner Review" || status === "Approved") ? <button className="button small ghost" disabled={!!busy} onClick={() => submit("reopen")}>{busy === "reopen" ? "Reopening..." : "Reopen"}</button> : null}
