...
export type PayrollReviewItem = { id: number; employee_id: number; employee_name: string; department: string; gross_pay: number; net_pay: number; total_deductions: number; regular_hours: number; regular_pay: number; approved_ot_hours: number; ot_pay: number; night_diff_hours: number; night_diff_pay: number; holiday_pay: number; paid_leave_pay: number; freelance_pay: number; other_earnings: number; late_minutes: number; undertime_minutes: number; unpaid_absence_days: number; sss_ee: number; philhealth_ee: number; pagibig_ee: number; tax: number; cash_advance_deduction: number; other_deductions: number; warnings?: string | null; leave_summary?: string[] };
