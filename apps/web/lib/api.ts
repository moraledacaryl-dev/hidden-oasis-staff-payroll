...
export type PayrollRun = { id: number; period_start: string; period_end: string; payout_date: string; run_label: string; status: string; prepared_by?: string | null; approved_by?: string | null; approved_at?: string | null; paid_at?: string | null; locked_at?: string | null; reopen_reason?: string | null; validation_summary?: string | null; created_at: string; totals?: { employees: number; gross_pay: number; net_pay: number; total_deductions: number } };
...
export type PayrollCorrection = { id: number; payroll_run_id: number; employee_id: number; employee_name?: string | null; department?: string | null; adjustment_type: "Earning" | "Deduction" | "Note" | string; amount: number; reason: string; apply_to_next_run: number; created_by?: string | null; created_at?: string | null };
export type PayrollCorrectionsResponse = { ok: boolean; items: PayrollCorrection[]; mode: string };
export type AppUser = { id: number; display_name: string; role: string; role_key: string; active: number; must_change_password: number; last_login_at?: string | null; created_at?: string | null };
