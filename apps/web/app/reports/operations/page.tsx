  ...
  return (
    <Shell allowedRoles={["owner", "supervisor"]}>
      <div className="page">
        ...
        <section className="card"><div className="panel-title"><h2>Attendance summary</h2></div><div className="table-wrap"><table><thead><tr><th>Date</th><th>Employee</th><th>Issue</th><th>In / Out</th><th>OT</th></tr></thead><tbody>{exceptions.map((item) => <tr key={item.id}><td>{item.work_date}</td><td>{item.full_name}</td><td>{item.is_absent ? "Absent" : item.attendance_status}</td><td>{item.actual_in || "—"} / {item.actual_out || "—"}</td><td>{item.ot_status || "None"}</td></tr>)}{exceptions.length === 0 ? <tr><td colSpan={5}>No open attendance items.</td></tr> : null}</tbody></table></div><p className="muted section-note">{reviews.length} review decision(s) recorded for this cutoff.</p></section>
      </div>
    </Shell>
  );
}
