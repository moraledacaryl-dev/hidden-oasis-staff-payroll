...
async function load():Promise<Result|null>{
  ...
  const response=await fetch(`${apiBaseUrl()}/api/v1/me/payroll`,{headers:{Authorization:`Bearer ${token}`,...(process.env.STAFF_PAYROLL_API_KEY?{"X-API-Key":process.env.STAFF_PAYROLL_API_KEY}:{})},cache:"no-store"});
