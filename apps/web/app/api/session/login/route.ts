...
export async function POST(request: Request) {
  ...
  result.cookies.set(ACCESS_TOKEN_COOKIE, data.access_token, { path: "/", httpOnly: true, sameSite: "lax" });
  result.cookies.set(ROLE_COOKIE, data.user.role_key, { path: "/", httpOnly: true, sameSite: "lax" });
  result.cookies.set(NAME_COOKIE, data.user.display_name, { path: "/", httpOnly: true, sameSite: "lax" });
  return result;
}
