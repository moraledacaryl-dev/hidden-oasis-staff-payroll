...
type SlipRun = { id: number; period_start: string; period_end: string; payout_date: string; run_label: string; status: string; employee_count: number; distributed_count: number; revision_of_run_id?: number | null; revision_treatment?: string | null; superseded_by_run_id?: number | null; totals?: { net_pay: number } };

function activePayslipRuns(runs: SlipRun[]): SlipRun[] {
  const byPeriod = new Map<string, SlipRun>();

  for (const run of runs) {
    const key = `${run.period_start}|${run.period_end}`;
  ...
  return Array.from(byPeriod.values()).sort((a, b) => {
    const dateCompare = String(b.period_end || "").localeCompare(String(a.period_end || ""));
...
export default async function PayslipDistributionPage({ searchParams }: { searchParams: Promise<{ run_id?: string }> }) {
  ...
  const activeRuns = activePayslipRuns(runs);
  const requestedRunId = Number(params.run_id || 0);
  const requestedIsActive = activeRuns.some((run) => run.id === requestedRunId);
  const selectedRunId = requestedIsActive ? requestedRunId : Number(activeRuns[0]?.id || 0);
  ...
  return (
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page payslip-page">
        <header className="page-header"><div className="grid"><span className="eyebrow">Payslip Distribution</span><h1>Approved payslips</h1><p className="muted">Distribution shows only the latest active payroll version for each period. Older payroll versions remain available from Payroll Runs for audit.</p></div><StatusBadge label={!activeRuns.length ? "No approved payroll yet" : pending ? `${pending} pending` : "Complete"} tone={!activeRuns.length || pending ? "warning" : "ok"} /></header>
