...
export default async function PayslipDistributionPage({ searchParams }: { searchParams: Promise<{ run_id?: string }> }) {
  ...
  try {
    ...
    return <Shell allowedRoles={["owner", "payroll", "supervisor"]}><div className="page"><section className="card"><strong>Payslips unavailable</strong><p className="muted">{error instanceof Error ? error.message : "Try again shortly."}</p></section></div></Shell>;
  }
  const selectedRunId = Number(params.run_id || runs[0]?.id || 0);
  let detail: SlipDetail | null = null;
  try {
    detail = selectedRunId ? await loadDetail(selectedRunId) : null;
  } catch (error) {
    return <Shell allowedRoles={["owner", "payroll", "supervisor"]}><div className="page"><section className="card"><strong>Payslips unavailable</strong><p className="muted">{error instanceof Error ? error.message : "Try again shortly."}</p></section></div></Shell>;
  }
    ...
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page payslip-page">
        <header className="page-header">
          <div className="grid"><span className="eyebrow">Payslip Distribution</span><h1>Approved payslips</h1></div>
          <StatusBadge label={pending ? `${pending} pending` : "complete"} tone={pending ? "warning" : "ok"} />
        </header>
        <section className="card">
          <div className="panel-title"><h2>Payroll period</h2>{items.length ? <PrintButton label="Print payslips" /> : null}</div>
        ...
        <section className="card">
          <div className="panel-title"><h2>Distribution list</h2></div>
