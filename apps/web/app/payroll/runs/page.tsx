...
export default async function PayrollRunsPage({ searchParams }: { searchParams: Promise<{ status?: string; start?: string; end?: string }> }) {
  ...
  return (
    <Shell allowedRoles={["owner", "payroll"]}>
      <div className="page">
        <header className="page-header"><div className="grid"><span className="eyebrow">Payroll Runs</span><h1>Run history</h1><p className="muted">Review, filter, audit, report, and open saved payroll runs. Release/pay remains disabled.</p></div></header>
        ...
        <section className="card"><div className="panel-title"><div><h2>Saved runs</h2><p className="muted">{filtered.length} shown of {runs.length} total.</p></div></div><div className="table-wrap"><table><thead><tr><th>ID</th><th>Period</th><th>Status</th><th>Prepared</th><th>Approved</th><th>Employees</th><th>Net</th><th>Open</th></tr></thead><tbody>{filtered.map((run) => (<tr key={run.id}><td>{run.id}</td><td>{run.period_start} to {run.period_end}</td><td><StatusBadge label={run.status} tone={statusTone(run.status)} /></td><td>{run.prepared_by || "—"}</td><td>{run.approved_by || "—"}</td><td>{run.totals?.employees ?? 0}</td><td>{peso(run.totals?.net_pay)}</td><td><div className="action-row"><Link href={`/payroll/runs/${run.id}`}>Review</Link><Link href={`/payroll/runs/${run.id}/reports`}>Report</Link><Link href={`/payroll/runs/${run.id}/audit`}>Audit</Link></div></td></tr>))}{filtered.length === 0 ? <tr><td colSpan={8}>No payroll runs match the filters.</td></tr> : null}</tbody></table></div></section>
      </div>
    </Shell>
  );
}
