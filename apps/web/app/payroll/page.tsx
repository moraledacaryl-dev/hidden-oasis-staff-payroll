    ...
    <Shell allowedRoles={["owner", "payroll"]}>
      <div className="page">
        ...
        <section className="grid cols-2">
          ...
          <div className="card"><div className="panel-title"><div><h2>Next steps</h2><p className="muted">Keep each action separate.</p></div></div><div className="action-list"><Link className="action-item" href={`/payroll/qa?period_start=${periodStart}&period_end=${periodEnd}`}><strong>Payroll QA actions</strong><p className="muted">Review schedule vs actual vs biometric mismatches before saving/finalizing payroll.</p></Link><Link className="action-item" href="/cutoff"><strong>Save draft</strong><p className="muted">Use Cutoff Control after QA is clean.</p></Link><Link className="action-item" href="/payroll/runs"><strong>Review runs</strong><p className="muted">Approve, reopen, or mark paid.</p></Link></div></div>
        </section>
        <section className="card"><div className="panel-title"><div><h2>Employee lines</h2><p className="muted">Current computed preview, including approved paid leave.</p></div></div><div className="table-wrap"><table><thead><tr><th>Employee</th><th>Regular hrs</th><th>Regular pay</th><th>Leave days</th><th>Leave pay</th><th>OT hrs</th><th>ND hrs</th><th>Gross</th><th>Deductions</th><th>CA deduction</th><th>Net</th><th>Warnings</th></tr></thead><tbody>{preview.items.map((item) => (<tr key={item.employee_id}><td><strong>{item.full_name}</strong><br /><span className="muted">{item.employee_code}</span></td><td>{numberText(item.regular_hours)}</td><td>{peso(item.regular_pay)}</td><td>{numberText(item.paid_leave_days)}</td><td>{peso(item.paid_leave_pay)}</td><td>{numberText(item.approved_ot_hours)}</td><td>{numberText(item.night_diff_hours)}</td><td>{peso(item.gross_pay)}</td><td>{peso(item.total_deductions)}</td><td>{peso(item.cash_advance_deduction)}</td><td><strong>{peso(item.net_pay)}</strong></td><td>{item.warnings?.length ? `${item.warnings.length} warning(s)` : "—"}</td></tr>))}</tbody></table></div></section>
      </div>
    </Shell>
  );
}
