...
export default async function SchedulePage({ searchParams }: { searchParams: Promise<{ week_start?: string; department?: string; position?: string; employee_id?: string }> }) {
  ...
  return (
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page">
        <header className="page-header"><div className="grid"><span className="eyebrow">Schedule</span><h1>{week.week_start} to {week.week_end}</h1></div><div className="badge-row"><StatusBadge label="planned + actual" tone="warning" /><PrintButton label="Download landscape" /></div></header>
        ...
        <section className={`card ${styles.screenSchedule}`}><div className="panel-title"><h2>Week</h2></div><div className={styles.boardScroll}><ScheduleBoardClient days={days} shifts={filteredItems} employees={boardEmployees} canEdit={canEditSchedule} /></div></section>
