...
export default async function SchedulePage({ searchParams }: { searchParams: Promise<{ week_start?: string; department?: string; position?: string; employee_id?: string }> }) {
  ...
  if (failed?.status === "rejected") {
    return <Shell allowedRoles={["owner", "payroll", "supervisor"]}><div className="page"><section className="card"><strong>Schedule unavailable</strong><p className="muted">{failed.reason instanceof Error ? failed.reason.message : "Try again shortly."}</p></section></div></Shell>;
  }
  ...
  return (
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page">
        <header className={`page-header ${styles.scheduleHero}`}><div className="grid"><span className="eyebrow">Schedule</span><h1>{week.week_start} to {week.week_end}</h1></div><div className="badge-row"><StatusBadge label="planned + actual" tone="warning" /></div></header>

        <section className={`card ${styles.calendarWorkspace} ${styles.screenSchedule}`}>
          <div className={styles.calendarHeader}>
            <div className={styles.weekSummary}><span className="eyebrow">Week board</span><h2>{selectedEmployee ? selectedEmployee.full_name : "All employees"}</h2><p className="muted">{filteredItems.length} shifts · {numberText(totalHours)} scheduled hrs · {actualRecorded} actual records</p></div>
