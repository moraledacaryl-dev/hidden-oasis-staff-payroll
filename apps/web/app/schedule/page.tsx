    ...
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page">
        <header className="page-header">
          <div className="grid"><span className="eyebrow">Schedule Board</span><h1>{week.week_start} to {week.week_end}</h1><p className="muted">Sling-style weekly schedule board. Planned hours are only the baseline before attendance, leave, and approved OT.</p></div>
          <StatusBadge label="planned only" tone="warning" />
        </header>

        <section className={styles.toolbar}>
          <Link className="primary-link" href={filterHref(selectedDepartment, selectedPosition, previousWeekStart)}>Previous week</Link>
          <Link className="primary-link" href={filterHref(selectedDepartment, selectedPosition, addWeek(week.week_start, 1))}>Next week</Link>
        ...
        <section className="grid cols-3"><div className="card metric"><span className="eyebrow">Visible shifts</span><strong className="metric-value">{filteredItems.length}</strong></div><div className="card metric"><span className="eyebrow">Planned paid hours</span><strong className="metric-value">{numberText(totalHours)}</strong></div><div className="card metric"><span className="eyebrow">Unassigned</span><strong className="metric-value">{unassignedCount}</strong></div></section>

        <section className="card"><div className="panel-title"><div><h2>Filters</h2><p className="muted">Filter without changing schedule data.</p></div></div><div className={styles.filterGrid}><div><strong>Department</strong><div className={styles.chips}><Link className={selectedDepartment === "all" ? styles.activeChip : styles.chip} href={filterHref("all", selectedPosition)}>All</Link>{departments.map((dept) => (<Link className={selectedDepartment === dept ? styles.activeChip : styles.chip} href={filterHref(dept, selectedPosition)} key={dept}>{dept}</Link>))}</div></div><div><strong>Position</strong><div className={styles.chips}><Link className={selectedPosition === "all" ? styles.activeChip : styles.chip} href={filterHref(selectedDepartment, "all")}>All</Link>{positions.map((pos) => (<Link className={selectedPosition === pos ? styles.activeChip : styles.chip} href={filterHref(selectedDepartment, pos)} key={pos}>{pos}</Link>))}</div></div></div></section>

        <section className="grid cols-2">
          <div className="card"><div className="panel-title"><div><h2>Add planned shift</h2><p className="muted">Creates a scheduled baseline only. It does not create payroll, time logs, OT, or released pay.</p></div></div><ScheduleShiftForm weekStart={week.week_start} employees={employees} /></div>
          <div className="card"><div className="panel-title"><div><h2>People visible</h2><p className="muted">Filtered scheduled employees and planned hours.</p></div></div><div className={styles.peopleList}>{Object.entries(peopleHours).length ? Object.entries(peopleHours).map(([person, stats]) => (<div className={styles.personRow} key={person}><strong>{person}</strong><span>{stats.shifts} shift{stats.shifts === 1 ? "" : "s"} · {numberText(stats.hours)} hrs</span></div>)) : <p className="muted">No scheduled people yet.</p>}</div></div>
        </section>

        <section className="card"><div className="panel-title"><div><h2>Week view</h2><p className="muted">Filtered calendar view. Copy-week is active; edit/delete can be added after the board is stable.</p></div></div><div className={styles.scheduleGrid}>{days.map((day) => { const shifts = filteredItems.filter((item) => item.shift_date === day); const dayHours = shifts.reduce((sum, item) => sum + Number(item.planned_paid_hours || 0), 0); return (<div className={styles.scheduleDay} key={day}><div className={styles.scheduleDayHead}><div><strong>{label(day)}</strong><span>{numberText(dayHours)} hrs</span></div><span>{shifts.length} shift{shifts.length === 1 ? "" : "s"}</span></div><div className={styles.scheduleStack}>{shifts.map((shift) => (<div className={styles.shiftCard} key={shift.id}><div className={styles.shiftTop}><strong>{shift.employee_name || "Unassigned"}</strong><span>{shift.position}</span></div><span>{shift.start_time}–{shift.end_time}{shift.is_overnight ? " +1" : ""}</span><span>{numberText(shift.planned_paid_hours)} paid hrs · break {shift.break_minutes}m</span><span>{shift.employee_department || shift.department || "No department"}</span>{shift.notes ? <p className="muted">{shift.notes}</p> : null}</div>))}{shifts.length === 0 ? <div className={styles.emptyDay}>No shifts</div> : null}</div></div>); })}</div></section>
      </div>
    </Shell>
  );
}
