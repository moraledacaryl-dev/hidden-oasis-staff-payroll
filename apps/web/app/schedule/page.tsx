...
function scheduleCellText(shifts: Shift[]) {
  if (!shifts.length) return "—";
  return shifts.map((shift) => `${shift.start_time}–${shift.end_time}${shift.is_overnight ? " +1" : ""}\n${shift.position || "Shift"}`).join("\n\n");
}
...
export default async function SchedulePage({ searchParams }: { searchParams: Promise<{ week_start?: string; department?: string; position?: string; employee_id?: string }> }) {
  ...
  const filteredItems = enrichedItems.filter((item) => {
    const employee = item.employee_id ? employees.find((record) => record.id === item.employee_id) : null;
    const dept = item.employee_department || item.department || employee?.department || "";
    const pos = item.position || employee?.position || "";
    const byDepartment = selectedDepartment === "all" || dept === selectedDepartment;
    const byPosition = selectedPosition === "all" || pos === selectedPosition;
  ...
  const boardEmployees = employees.filter((employee) => {
    const byDepartment = selectedDepartment === "all" || employee.department === selectedDepartment;
    const byPosition = selectedPosition === "all" || employee.position === selectedPosition || filteredItems.some((item) => item.employee_id === employee.id && item.position === selectedPosition);
    ...
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page">
        <header className="page-header">
          <div className="grid"><span className="eyebrow">Schedule</span><h1>{week.week_start} to {week.week_end}</h1><p className="muted">Scheduled and actual attendance are shown together.</p></div>
          <div className="badge-row"><StatusBadge label="planned + actual" tone="warning" /><PrintButton label="Download landscape" /></div>
        </header>
