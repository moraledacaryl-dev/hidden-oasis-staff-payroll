  ...
  return (
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page">
        ...
        <section className="card"><div className="panel-title"><div><h2>Add planned shift</h2><p className="muted">This creates a scheduled baseline shift only. It does not create payroll, time logs, OT, or release pay.</p></div></div><ScheduleShiftForm weekStart={week.week_start} employees={employees} /></section>
        <section className="card"><div className="panel-title"><div><h2>Week view</h2><p className="muted">Next update can add drag-and-drop. Current edit path is the add-shift form above.</p></div></div><div className={styles.scheduleGrid}>{days.map((day) => { const shifts = week.items.filter((item) => item.shift_date === day); return (<div className={styles.scheduleDay} key={day}><div className={styles.scheduleDayHead}><strong>{label(day)}</strong><span className="muted">{shifts.length} shift{shifts.length===1?"":"s"}</span></div><div className={styles.scheduleStack}>{shifts.map((shift)=>(<div className={styles.shiftCard} key={shift.id}><strong>{shift.employee_name || "Unassigned"}</strong><span>{shift.position}</span><span>{shift.start_time}–{shift.end_time}{shift.is_overnight ? " +1" : ""}</span><span>{numberText(shift.planned_paid_hours)} paid hrs · break {shift.break_minutes}m</span>{shift.notes ? <p className="muted">{shift.notes}</p> : null}</div>))}{shifts.length===0 ? <div className={styles.emptyDay}>No shifts</div> : null}</div></div>); })}</div></section>
      </div>
    </Shell>
  );
}
