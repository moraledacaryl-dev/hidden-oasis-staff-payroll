  ...
  return (
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page">
        ...
        <section
          className="card"
          data-cutoff-selector
          data-period-start={periodStart}
          data-period-end={periodEnd}
          data-period-complete={periodComplete}
        >
          <form className="form-row" method="get">
            <div className="field">
              <label htmlFor="cutoff-month">Month</label>
              <input id="cutoff-month" max={today.slice(0, 7)} name="month" type="month" defaultValue={selected.month} />
            </div>
            <div className="field">
              <label htmlFor="cutoff-half">Period</label>
              <select id="cutoff-half" name="half" defaultValue={selected.half}>
                <option value="first">1st to 15th</option>
                <option value="second">16th to month end</option>
              </select>
            </div>
        ...
        {canSeePayroll && preview ? <section className="grid cols-3"><div className="card"><strong>Draft payroll run</strong><p className="muted">Creates a saved draft. Approval stays separate.</p><PayrollDraftButton periodStart={periodStart} periodEnd={periodEnd} payoutDate={payoutDate} disabledReason={periodComplete ? undefined : `Available after ${periodEnd}.`} /></div><div className="card"><strong>Gross / net</strong><p>{peso(preview.totals.gross_pay)} gross</p><p>{peso(preview.totals.net_pay)} net</p></div><div className="card"><strong>Readiness</strong><p className="muted">{ready ? "Ready for owner review." : "Clear blockers first."}</p></div></section> : null}
