...
function previewWarningText(warnings: string[] | undefined) {
...
export default async function CutoffPage({ searchParams }: CutoffPageProps) {
  ...
  const hasDraftForPeriod = matchingRuns.length > 0;
    ...
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page cutoff-page">
        ...
        <section className="cutoff-metrics">
          {preview ? <div className={metricTone("default")}><span>Net payroll</span><strong>{peso(netPay)}</strong><small>Live preview</small></div> : <div className={metricTone("default")}><span>Payroll</span><strong>Restricted</strong><small>Operations view</small></div>}
        ...
        {canSeePayroll && preview ? <section className="cutoff-action-grid"><div className="card cutoff-draft-card" data-create-draft="true" data-create-payroll-draft="true"><div><span className="eyebrow">Next action</span><h2>{hasDraftForPeriod ? "Saved payroll exists" : "Draft payroll run"}</h2><p className="muted">{hasDraftForPeriod ? "Use the saved run below for lifecycle actions." : "Create a saved payroll draft after reviewing the live preview."}</p></div>{hasDraftForPeriod ? null : <PayrollDraftButton periodStart={periodStart} periodEnd={periodEnd} payoutDate={payoutDate} />}</div><div className="card cutoff-total-card"><span className="eyebrow">Payroll total</span><h2>{peso(grossPay)} gross</h2><p>{peso(netPay)} net after deductions</p></div></section> : null}

        {canSeePayroll && preview ? <section className="card"><div className="panel-title"><div><span className="eyebrow">Live payroll</span><h2>Payroll Preview</h2><p className="muted">Computed live for this cutoff. No draft is required to review these numbers.</p></div><StatusBadge label={hasDraftForPeriod ? "saved run exists" : "preview only"} tone={hasDraftForPeriod ? "ok" : "warning"} /></div>{previewItems.length ? <div className="table-wrap"><table><thead><tr><th>Employee</th><th>Gross</th><th>Deductions</th><th>Cash Adv.</th><th>Net</th><th>Flags</th></tr></thead><tbody>{previewItems.map((item) => (<tr key={`preview-${item.employee_id}`}><td><strong>{item.full_name}</strong><p className="muted">{item.employee_code || "—"}</p></td><td>{peso(item.gross_pay)}<p className="muted">Reg {numberText(item.regular_hours)}h · OT {numberText(item.approved_ot_hours)}h</p></td><td>{peso(item.total_deductions)}<p className="muted">SSS/PH/Pag-IBIG/Tax included</p></td><td>{item.cash_advance_deduction ? peso(item.cash_advance_deduction) : "—"}</td><td><strong>{peso(item.net_pay)}</strong></td><td><p className="muted">{previewWarningText(item.warnings)}</p></td></tr>))}</tbody></table></div> : <div className="empty-state"><strong>No preview rows</strong><p>No employee payroll rows were computed for this cutoff.</p></div>}</section> : null}
