...
export default async function StaffPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; department?: string; saved?: string }> }) {
  ...
  return (
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page people-page">
        ...
        <MobileSection title="Workforce totals" description={`${active} active · ${onLeave} on leave`}><section className="people-kpis">
        ...
        {params.saved === "1" ? <p className="form-feedback is-success" role="status">Worker record saved.</p> : null}
        <SectionCard>
          <Toolbar>
            <form className="people-filter" action="/staff">
              ...
              <button className="button" type="submit">Apply</button><Link className="button secondary" href="/staff">Reset</Link>
            </form>
            <div className="people-tabs"><Link aria-current={status === "current" ? "page" : undefined} href={`/staff?${new URLSearchParams({ status: "current", q: params.q || "", department }).toString()}`}>Current</Link><Link aria-current={status === "former" ? "page" : undefined} href={`/staff?${new URLSearchParams({ status: "former", q: params.q || "", department }).toString()}`}>Former</Link><Link aria-current={status === "all" ? "page" : undefined} href={`/staff?${new URLSearchParams({ status: "all", q: params.q || "", department }).toString()}`}>All</Link></div>
          </Toolbar>
        </SectionCard>
        ...
        <section className="people-directory">
          ...
          <div className="people-table-wrap">
            <table className="people-table responsive-records">
              ...
              <tbody>
                {filtered.map((employee) => <tr key={employee.id}>
                  <td data-label="Worker"><div className="people-identity"><span className="people-avatar">{initials(employee.full_name)}</span><div><strong>{employee.full_name}</strong><small>{employee.employee_code}</small></div></div></td>
                  <td data-label="Department">{employee.department_name || "Unassigned"}</td>
                  <td data-label="Role">{employee.position || "—"}</td>
                  <td data-label="Engagement">{employee.employment_type || "—"}</td>
                  <td data-label="Lifecycle"><StatusBadge label={employee.status} tone={employee.status === "Active" ? "ok" : employee.status === "On Leave" ? "warning" : "neutral"} /></td>
                  <td data-label="Default shift">{employee.default_shift_start || "—"}–{employee.default_shift_end || "—"}</td>
                  {showPrivate ? <td data-label="Benefits"><div className="badge-row">{employee.benefits_sss ? <span className="badge">SSS</span> : null}{employee.benefits_philhealth ? <span className="badge">PhilHealth</span> : null}{employee.benefits_pagibig ? <span className="badge">Pag-IBIG</span> : null}{employee.benefits_tax ? <span className="badge">Tax</span> : null}</div></td> : null}
                  {canEdit ? <td data-label="Action"><div className="people-actions"><Link className="button small ghost" href={`/staff/manage?employee=${employee.id}#employee-${employee.id}`}>Open</Link></div></td> : null}
                </tr>)}
        ...
        <MobileSection title="Related workspaces" description="HR records and employee financial support"><section className="people-card-grid">
          <SectionCard><SectionHeader title="People operations" description="Related records and management workspaces." /><SectionBody><div className="people-list">{session.role_key !== "payroll" ? <Link className="people-list-row" href="/performance-reviews"><span><strong>Performance reviews</strong><span>Annual reviews and follow-up records</span></span><strong>Open →</strong></Link> : null}<Link className="people-list-row" href="/hr"><span><strong>HR and leave</strong><span>Entitlements, requests, and formal records</span></span><strong>Open →</strong></Link></div></SectionBody></SectionCard>
