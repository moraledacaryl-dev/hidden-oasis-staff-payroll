...
import { EmptyState, MetricCard, MetricGrid, Notice, PageHeading, SectionBody, SectionCard, SectionHeader, Toolbar } from "@/components/UiPrimitives";
  ...
  return (
    <Shell allowedRoles={["owner", "supervisor"]}>
      <div className={`page ${styles.page}`}>
        <PageHeading eyebrow="Attendance workspace" title="Attendance review" description={`${periodStart} to ${periodEnd}. Review only meaningful exceptions, then correct source records in Schedule.`} actions={<><Link className="button secondary" href="/schedule/import">Upload logs</Link><Link className="button" href="/schedule">Correct in Schedule</Link></>} />
        ...
        {!compliance.ok ? <Notice tone="danger"><strong>Attendance data did not load.</strong><br />{compliance.error || "The compliance API returned an error."}</Notice> : null}

        <MetricGrid><MetricCard value={lateCount} label="Late infractions" /><MetricCard value={missingCount} label="Missing logs" /><MetricCard value={partialCount} label="Partial absences" /><MetricCard value={absenceCount} label="Absences, leave, or AWOL" /></MetricGrid>

        <section className={styles.overview}>
          <SectionCard>
            <SectionHeader title="Priority review queue" description="Employees with a handbook action or material attendance exception." actions={<StatusBadge label={actionItems.length ? `${actionItems.length} need action` : "Clear"} tone={actionItems.length ? "warning" : "ok"} />} />
            <SectionBody><div className={styles.attentionList}>{actionItems.slice(0, 5).map((item) => <div className={styles.attentionItem} key={item.employee_id}><span className={styles.attentionIcon}>!</span><div><strong>{item.full_name}</strong><p>{item.handbook_action} · {Number(item.late_infractions || 0)} late · {Number(item.missing_logs || 0)} missing</p></div><AttendanceMemoForm employeeId={Number(item.employee_id)} employeeName={String(item.full_name || "Employee")} periodMonth={month} suggestedAction={String(item.handbook_action || "Attendance infraction")} /></div>)}{actionItems.length === 0 ? <EmptyState title="No attendance actions" description="The selected month is clear." /> : null}</div></SectionBody>
          </SectionCard>

          <SectionCard>
            <SectionHeader eyebrow="Month pulse" title={month} />
            <SectionBody><div className={styles.sideStats}><div className={styles.sideRow}><span>Employees shown</span><strong>{complianceItems.length}</strong></div><div className={styles.sideRow}><span>Grace periods</span><strong>{graceCount}</strong></div><div className={styles.sideRow}><span>Memos issued</span><strong>{memos.length}</strong></div><div className={styles.sideRow}><span>Action required</span><strong>{actionItems.length}</strong></div></div><Link className="button secondary" href="/performance-reviews">Open performance reviews</Link></SectionBody>
          </SectionCard>
        </section>

        <SectionCard>
          <SectionHeader title="Monthly compliance" description="Detailed operational record for the selected month." actions={<StatusBadge label={`${complianceItems.length} employees`} />} />
          <SectionBody flush><div className={styles.tableWrap}><table className="compliance-table"><thead><tr><th>Employee</th><th>Scheduled</th><th>Missing</th><th>Grace</th><th>Lates</th><th>Partial</th><th>Absences</th><th>Details</th><th>Handbook action</th><th>Memo</th></tr></thead><tbody>{complianceItems.map((item) => <tr key={item.employee_id}><td className={styles.employeeCell}><strong>{item.full_name}</strong><br /><span className="muted">{item.employee_code || "—"} · {item.department || "—"}</span></td><td>{Number(item.scheduled_shifts || 0)}</td><td>{Number(item.missing_logs || 0)}</td><td>{Number(item.grace_periods || 0)}</td><td>{Number(item.late_infractions || 0)}</td><td>{Number(item.partial_absences || 0)}</td><td>Approved: {Number(item.approved_absences || 0)}<br />Unexcused: {Number(item.unexcused_absences || 0)}<br />AWOL: {Number(item.awol || 0)}</td><td>{(item.late_details || []).length || (item.absence_details || []).length ? <details className="compliance-details"><summary>View details ({(item.late_details || []).length + (item.absence_details || []).length})</summary>{(item.late_details || []).length ? <div className="compliance-details-list"><strong>Lates</strong>{(item.late_details || []).map((late) => <div key={`${late.date}-${late.actual_in}`} className="muted">{late.date}: {late.scheduled_start} → {late.actual_in} ({late.minutes_late} min, {late.status})</div>)}</div> : null}{(item.absence_details || []).length ? <div className="compliance-details-list"><strong>Absences</strong>{(item.absence_details || []).map((absence) => <div key={`${absence.date}-${absence.type}`} className="muted">{absence.date}: {absence.type}</div>)}</div> : null}</details> : <span className="muted">—</span>}</td><td className={styles.statusCell}><strong>{item.handbook_action}</strong><br /><span className="muted">{item.attendance_reward_status}</span></td><td>{item.handbook_action !== "No action required" ? <AttendanceMemoForm employeeId={Number(item.employee_id)} employeeName={String(item.full_name || "Employee")} periodMonth={month} suggestedAction={String(item.handbook_action || "Attendance infraction")} /> : <span className="muted">—</span>}</td></tr>)}{complianceItems.length === 0 ? <tr><td colSpan={10}>No monthly attendance records found.</td></tr> : null}</tbody></table></div></SectionBody>
        </SectionCard>

        {performance ? <SectionCard><SectionHeader title="12-month staff performance" description={performance.employee?.full_name || "Selected employee"} /><SectionBody><div className={styles.performanceMetrics}><div className={styles.performanceMetric}><strong>{performance.totals?.late_infractions || 0}</strong><span>Total lates</span></div><div className={styles.performanceMetric}><strong>{performance.totals?.grace_periods || 0}</strong><span>Grace periods</span></div><div className={styles.performanceMetric}><strong>{performance.totals?.unexcused_absences || 0}</strong><span>Unexcused absences</span></div><div className={styles.performanceMetric}><strong>{performance.totals?.awol || 0}</strong><span>AWOL</span></div></div><div className="table-wrap"><table><thead><tr><th>Month</th><th>Scheduled</th><th>Missing</th><th>Grace</th><th>Lates</th><th>Partial</th><th>Unexcused</th><th>AWOL</th><th>Approved</th><th>Action</th></tr></thead><tbody>{(performance.months || []).map((row) => <tr key={row.month}><td>{row.month}</td><td>{row.scheduled_shifts}</td><td>{row.missing_logs}</td><td>{row.grace_periods}</td><td>{row.late_infractions}</td><td>{row.partial_absences}</td><td>{row.unexcused_absences}</td><td>{row.awol}</td><td>{row.approved_absences}</td><td>{row.handbook_action}</td></tr>)}</tbody></table></div></SectionBody></SectionCard> : null}

        <SectionCard><SectionHeader title="Memo history" description="Attendance memos issued for the selected month." /><SectionBody flush><div className="table-wrap"><table><thead><tr><th>Issued</th><th>Employee</th><th>Type</th><th>Level</th><th>Status</th><th>Reason</th><th>Notes</th></tr></thead><tbody>{memos.map((memo) => <tr key={memo.id}><td>{memo.issued_at || memo.created_at || "—"}</td><td><strong>{memo.full_name || "—"}</strong><br /><span className="muted">{memo.employee_code || "—"} · {memo.department || "—"}</span></td><td>{memo.memo_type}</td><td>{memo.memo_level}</td><td>{memo.status}</td><td>{memo.reason}</td><td>{memo.notes || "—"}</td></tr>)}{memos.length === 0 ? <tr><td colSpan={7}>No attendance memos for this month.</td></tr> : null}</tbody></table></div></SectionBody></SectionCard>
      </div>
    </Shell>
  );
}
