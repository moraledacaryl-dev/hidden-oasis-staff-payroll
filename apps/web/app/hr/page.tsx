...
export default async function HrPage() {
  ...
  return <Shell allowedRoles={["owner", "payroll", "supervisor"]}><div className="page people-page">
    <PageHeading eyebrow="People" title="HR and leave" description="Manage leave requests, employee entitlements, formal records, and long-term people history." actions={<><Link className="button secondary" href="/staff">Staff directory</Link>{session.role_key !== "payroll" ? <Link className="button secondary" href="/performance-reviews">Performance reviews</Link> : null}</>} />
    ...
    <div className="hr-overview">
      <div className="hr-stack">
        ...
        <MobileSection title="Leave balances" description="Credits and usage by employee"><section className="hr-panel"><header><div><h2>Leave balances</h2><p>Credits, usage, remaining balance, and effective entitlement window.</p></div><StatusBadge label={`${leaveEmployees.length} employees`} /></header><div className="hr-panel-body"><div className="hr-balance-list">{leaveEmployees.map((employee) => <div className="hr-balance-row" key={employee.id}><div><strong>{employee.full_name}</strong><small>{employee.employee_code || "—"} · {employee.department || "Unassigned"}</small></div><div>{employee.balances.length ? employee.balances.map((balance) => <div key={balance.leave_type_id}><strong>{balance.leave_type_name}: {num(balance.remaining)} left</strong><small>{num(balance.used)} used of {num(balance.credits)} · {entitlementWindow(balance)}</small></div>) : <span className="muted">No entitlement set</span>}</div></div>)}{!leaveEmployees.length ? <div className="people-empty">No leave balance records found.</div> : null}</div></div></section></MobileSection>
      </div>

      <div className="hr-stack">
        {canCreate ? <MobileSection title="Add HR record" description="Memo, infraction, or annual review"><section className="hr-panel"><header><div><h2>Add HR record</h2><p>Memo, infraction, or annual review.</p></div></header><div className="hr-panel-body"><HrRecordForm employees={employees} /></div></section></MobileSection> : null}
        {canManageEntitlements ? <MobileSection title="Set leave entitlement" description="Credits, eligibility, and effective dates"><section className="hr-panel"><header><div><h2>Set leave entitlement</h2><p>Credits, eligibility, and effective dates.</p></div></header><div className="hr-panel-body"><LeaveEntitlementForm employees={employees} leaveTypes={leaveTypes} defaultYear={leaveBalanceResponse.year} /></div></section></MobileSection> : null}
      </div>
    </div>

    <SectionCard><SectionHeader title="HR record timeline" description="Formal employee records remain available as an auditable history." actions={<StatusBadge label={`${records.length} records`} />} /><SectionBody flush><div className="people-table-wrap"><table className="hr-record-table responsive-records"><thead><tr><th>Date</th><th>Employee</th><th>Type</th><th>Subject</th><th>Severity</th><th>Status</th><th>Issued by</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td data-label="Date">{record.record_date}</td><td data-label="Employee"><strong>{record.employee_name}</strong><br /><span className="muted">{record.department || "—"} · {record.position || "—"}</span></td><td data-label="Type">{record.record_type}</td><td data-label="Subject">{record.subject}{record.rating != null ? ` · Rating ${record.rating}` : ""}</td><td data-label="Severity">{record.severity}</td><td data-label="Status"><StatusBadge label={record.status} tone={["Closed", "Resolved", "Finalized"].includes(record.status) ? "ok" : "warning"} /></td><td data-label="Issued by">{record.issued_by || "—"}</td></tr>)}{!records.length ? <tr><td colSpan={7}>No HR records yet.</td></tr> : null}</tbody></table></div></SectionBody></SectionCard>
  </div></Shell>;
}
