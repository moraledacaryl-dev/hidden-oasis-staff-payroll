...
export default async function HrPage() {
  ...
  return (
    <Shell allowedRoles={["owner", "payroll", "supervisor"]}>
      <div className="page">
        <header className="page-header"><div><span className="eyebrow">HR Records</span><h1>Leave and formal records</h1><p className="muted">Management-only records. Staff self-service is limited to the logged-in employee.</p></div></header>
        <section className="grid cols-3"><div className="card"><strong>Formal records</strong><p>{records.length}</p></div><div className="card"><strong>Infractions</strong><p>{infractions}</p></div><div className="card"><strong>Memos</strong><p>{memos}</p></div></section>
        {canCreate ? <section className="card"><div className="panel-title"><div><h2>Add formal HR record</h2><p className="muted">Create infraction, memo, leave note, or HR document.</p></div></div><HrRecordForm employees={employees} /></section> : null}
        <section className="card"><div className="panel-title"><div><h2>Leave balances</h2></div></div><div className="table-wrap"><table><thead><tr><th>Employee</th><th>Department</th><th>Leave balances</th></tr></thead><tbody>{leaveEmployees.map((employee) => <tr key={employee.id}><td>{employee.full_name}</td><td>{employee.department || "—"}</td><td>{employee.balances.length ? employee.balances.map((b) => `${b.leave_type_name}: ${num(b.remaining)} remaining / ${num(b.credits)} credits`).join(" · ") : "No entitlement set"}</td></tr>)}{leaveEmployees.length === 0 ? <tr><td colSpan={3}>No leave balance records found.</td></tr> : null}</tbody></table></div></section>
        <section className="card"><div className="panel-title"><div><h2>HR record timeline</h2></div></div><div className="table-wrap"><table><thead><tr><th>Date</th><th>Employee</th><th>Type</th><th>Subject</th><th>Severity</th><th>Status</th><th>Issued by</th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td>{record.record_date}</td><td>{record.employee_name}</td><td>{record.record_type}</td><td>{record.subject}{record.rating != null ? ` · Rating ${record.rating}` : ""}</td><td>{record.severity}</td><td>{record.status}</td><td>{record.issued_by || "—"}</td></tr>)}{records.length === 0 ? <tr><td colSpan={7}>No HR records yet.</td></tr> : null}</tbody></table></div></section>
      </div>
    </Shell>
  );
}
