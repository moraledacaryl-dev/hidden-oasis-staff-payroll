...
import { EmptyState, PageHeading, SectionBody, SectionCard, SectionHeader } from "@/components/UiPrimitives";
...
function Kpi({ code, label, value, foot, trend, warning = false }: { code: string; label: string; value: string | number; foot: string; trend?: string; warning?: boolean }) {
  return <section className={styles.kpiCard}><div className={styles.kpiTop}><span className={styles.kpiIcon}>{code}</span>{trend ? <StatusBadge label={trend} tone={warning ? "warning" : "ok"} /> : null}</div><div className={styles.kpiLabel}>{label}</div><div className={styles.kpiValue}>{value}</div><div className={styles.kpiFoot}>{foot}</div></section>;
}
